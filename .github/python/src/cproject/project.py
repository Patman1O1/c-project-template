...
class Project(object):
    ...
    def __init__(self,
        ...
        self.env: Environment = Environment(
            loader=FileSystemLoader(self.root),   # root must match _render's relative_to
    ...
    def _render(self, filepath: Path, cmake: CMake) -> None:  # raises ValueError, jinja2.TemplateNotFound
        name: str = filepath.resolve().relative_to(self.root.resolve()).as_posix()
    ...
    def render(self, path: Path, cmake: CMake) -> None:  # raises NotADirectoryError, ValueError, jinja2.TemplateNotFound
        # A single file: render it only if it is a template
        if path.is_file():
            if path.suffix == ".j2":
                self._render(path, cmake)
            return
        ...
        for filepath in sorted(path.rglob("*.j2")):
